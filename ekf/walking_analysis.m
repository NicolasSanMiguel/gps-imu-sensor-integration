...
basicGyroPlots = false;
headingOverTime = true;
plotAccVals = false;
plotAccVelocityVals = false;
plotGPS3D = false;
...
% normalize gyro data based on datasheet
gX = gX/250;
gY = gY/250;
gZ = gZ/250;
...
for i = 1:length(gdat)
headings(i+1) = wrapTo360( headings(i) + gdat(i) );
end

figure
% plot(xvals/10,headings)
xvals = [1:length(headings)];
polarplot(deg2rad(headings),xvals/10)
end
...
gX = gX(1:length(timeIMU));
gY = gY(1:length(timeIMU));
gZ = gZ(1:length(timeIMU));
GPSx = GPSx(1:length(timeGPS));
GPSy = GPSy(1:length(timeGPS));
GPSz = GPSz(1:length(timeGPS));
GPSlat = GPSlat(1:length(timeGPS));
GPSlong = GPSlong(1:length(timeGPS));
GPSalt = GPSalt(1:length(timeGPS));


save('walking_dataEKF')
...
if plotGPS3D
    n = 203;
figure
plot3(GPSx(1:n),GPSy(1:n),GPSz(1:n))
